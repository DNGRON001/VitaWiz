//get data
db.collection('Jobseeker').where("Public","==", true).get().then(snapshot => {
...
signupForm.addEventListener('submit', (e) => {
  ...
  if (document.getElementById('signup-password').value ==
    ...
    auth.createUserWithEmailAndPassword(email, password).then(cred => {
      ...
      db.collection("test").doc(cred.user.uid).set({
        AwaitLoad: true,
        SecondViewing: false,
        edit: false,
        editnum: ""
      ...
      db.collection("test").doc(cred.user.uid)
        .collection("testCV").doc("CV")
        .set({
        Name: '',
        Surname: '',
        Phone: '',
...
window.onload = function(){
  var a = document.getElementById("Forgot");

  a.onclick = function(){
    var auth = firebase.auth();
    var emailAddress = window. prompt("Enter your email address: ")

      auth.sendPasswordResetEmail(emailAddress).then(function() {
        alert("We have sent you a link that allows you to reset your password. Please check your email.");
        // Email sent.
      }).catch(function(error) {
